...
### Added
 ...
 - sub-graph isolation:
   - now you can isolate a sub-graph of one node into a separate node.
   - the sub-graph in the original node will be replaced by a `Call` to the new node.
   - the new node will be `internal` by default.
 - auto-naming:
